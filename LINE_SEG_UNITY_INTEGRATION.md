...
Companion docs: [`SpanUNet3D_Semantic.md`](../SpanUNet3D_Semantic.md) (model + metrics), [`README_GOAL2.md`](../README_GOAL2.md) (training pipeline), [`PAPERSPACE_UPLOAD_AND_RUN.md`](../PAPERSPACE_UPLOAD_AND_RUN.md) (Paperspace upload/run).
...
| `post_processing.class_to_bmp_uint8` | object | Maps class index → dataset BMP gray. Air/solid are literal `255` / `0`; line classes are `pack_line_gray(16, type_code)`, i.e. `(16 << 3) \| type_code = 128..132`. |
| `post_processing.single_line_type_enforcement` | string | Documents the constraint that each span holds at most one conductor type and how to enforce it in C# after argmax. |
...
| Enforce Single Line Type Per Span | `true` (default). Keep enabled — each span contains at most one conductor type. |
...
public class LineSegExample : MonoBehaviour
{
    ...
    {
        // 1. Normalize bytes (0..255) → floats (0..1) and strip any embedded
        //    line-label pixels (128-254) so the model sees the same input as training.
        float[] volume = LineSegSentisInferenceManager.NormalizeAndStripFrames(uint8FramesTHW);

        // 2. Inference — single-type enforcement is applied automatically inside
        //    (Inspector field enforceSingleLineTypePerSpan = true).
        ...
        // 3. Check single-type enforcement diagnostics.
        if (result.singleTypeEnforced)
            Debug.Log(
                $"Single-type enforcement: kept class {result.keptLineClass}, " +
                $"suppressed {result.suppressedPixels} px");

        // 4. Optional: write predicted BMPs (one per frame) — same encoding as
...
The `result.classes` byte array is the same as the Python `predicted_semantic_classes.npy`: row-major `(T, H, W)` with values `0..6`. `result.bmpGray` is what `tools/bmp_io.py::write_bmp_gray` would produce in `infer_goal2.py`. Both outputs reflect the single-type enforcement step — at most one line class will be present across the whole volume.
...
```mermaid
flowchart LR
    BMPs["Input BMPs / floats T,H,W"] --> Norm["NormalizeAndStripFrames\n/255, strip 128–254→0"]
    ...
    Argmax --> Enforce["EnforceSingleLineType\nkeep dominant class only"]
    Enforce --> Classes["Per-voxel class 0..6\n(at most 1 line type)"]
    Classes --> BmpMap["ClassToBmpGray per voxel"]
...
For any span, the Unity output should byte-for-byte match `infer_goal2.py`'s `predicted_bmp/<frame>.bmp` and `predicted_semantic_classes.npy` (subject to FP precision differences if you use the FP16 ONNX). Both the Python script and the C# manager now apply the same single-type enforcement step after argmax, so the outputs remain consistent. To compare:

1. Run `python3 -m line_seg.infer_goal2 --weights ... --input_dir ... --output_dir ...` on the same span.
2. Run the Unity manager on the same uint8 frames (divided by 255 via `NormalizeAndStripFrames()`), with `enforceSingleLineTypePerSpan = true`.
...
> **Note:** If you run the Python script with `--allow_multi_type`, the outputs will diverge for spans where the model produced multi-type pixels. Keep both sides in sync (both enforce or both allow).
...
- [ ] `Enforce Single Line Type Per Span` is **checked** (default true) — enforces the one-conductor-per-span constraint.
- [ ] First end-to-end inference logs `[LineSeg] T=... total=... ms` with no errors.
- [ ] For spans where multi-type conflict occurred, the log shows `[single-type kept=N suppressed=N px]`.
