...
Related reading: [`README_GOAL2.md`](README_GOAL2.md), [`SpanUNet3D_Semantic.md`](SpanUNet3D_Semantic.md), [`README_CATENARY_BASELINE.md`](README_CATENARY_BASELINE.md), [`tools/line_encoding.py`](tools/line_encoding.py), [`tools/catenary_ransac.py`](tools/catenary_ransac.py).
...
## 11. Empirical evidence from trained models — the annotation ceiling in practice

The preceding sections describe what better annotation would enable. This section records what the trained `SpanUNet3D` model achieved on the **existing** grayscale-encoded labels and where it stopped improving — confirming that the annotation quality, not the model architecture, is now the binding constraint.

### 11.1 Training progression

Three training runs were executed on the same 230-span dataset (196 train / 34 val), each building on the previous:

| Run | Key settings | Best val lineIoU (cls 2–6) | Comm IoU | Primary IoU | Neutral IoU | Line-object F1 |
|-----|-------------|---------------------------|----------|-------------|-------------|----------------|
| **exp_v1** (baseline) | focal γ=2.0, uniform boost 2×, 30 epochs | 0.000 | 0.000 | 0.000 | 0.000 | — |
| **exp_v2** | focal γ=2.0, uniform boost 20×, checkpoint on lineIoU, 40 epochs | 0.5013 | 0.786 | 0.945 | 0.775 | 0.846 |
| **exp_v3** | focal γ=2.0, per-class weights (comm=25×, primary=10×, neutral=25×), label smoothing ε=0.05, patience=6, 60 epochs | **0.5102** | **0.818** | **0.949** | **0.785** | **0.932** |

### 11.2 Where the model converged — and why it stopped

After exp_v3, the model showed unambiguous convergence:

- Best `val_line_mean_iou` at epoch 49, with **11 epochs of no improvement** before training ended at epoch 60.
- `val_line_mean_iou` stdev over the last 10 epochs: **0.0052** — oscillation is noise, not learning.
- Val loss flat for the last 20 epochs (mean 0.519, std 0.0035) — no overfitting, no further learning.

The remaining inter-class confusion after exp_v3 is:

| Confusion | Rate | Trend vs exp_v2 |
|-----------|------|-----------------|
| comm → neutral | 8.4% | ↓ from 9.7% |
| neutral → primary | 8.1% | ↓ from 8.8% |
| neutral → comm | 4.8% | ↓ from 5.1% |

These numbers reduced each time the training was improved, but they **did not approach zero** and they stopped responding to further loss engineering. They represent the floor reachable with single-voxel grayscale intensity labels.

### 11.3 Why this is an annotation problem, not a model problem

The three classes involved in the remaining confusion — comm, primary, and neutral — share the following properties in the current label format:

- Their only distinguishing signal is a **3-bit type_code** (bits 0–2 of the gray byte). The raw intensity value carries no physically meaningful per-conductor information beyond that.
- In many spans, all three conductor types run **parallel and adjacent** in the same voxel slab. The spatial neighbourhood of a comm voxel is nearly identical to that of a neutral voxel.
- The existing labels contain **no instance identity** that crosses frames. The model cannot learn "conductor A stays comm across all T frames" because the label carries no such guarantee (a different instance_id may appear in adjacent frames due to manual annotation inconsistency).

Further training, wider models, or stronger loss functions cannot resolve confusion that is not resolvable from the label signal. The 8% comm↔neutral confusion is the irreducible floor for the current annotation scheme.

### 11.4 Which annotation improvements from this document would close the gap

Of the improvements described in sections 1–10, the following have the most direct impact on the observed confusion:

| Improvement | Section | How it addresses the 8% floor |
|------------|---------|-------------------------------|
| **Per-span `conductors.json`** with stable `instance_id` | §5 | Allows the model to learn "this is a comm conductor through all T frames", providing a cross-frame identity signal absent today |
| **3-click catenary primitive** per conductor | §2.1 | Ensures each conductor in the volume is explicitly annotated once, eliminating per-frame ambiguity where the annotator might assign different type codes to the same wire |
| **Coverage view** (all line voxels accounted for) | §3.2 | Prevents unannotated line voxels from being silently treated as solid by `raw_to_semantic_labels`, which adds noise to the neutral/comm boundary |
| **Type-vs-voxel cross-check** | §9 (QA gate 4) | Catches spans where the labeled type_code in the gray byte disagrees with the annotator's intended type — a known source of comm/neutral labeling errors |
| **16-bit PNG label** with 11-bit instance field | §7 | Makes it possible to train an instance-aware head alongside the semantic head, so the model learns to separate individual conductors rather than just classify their type |

In short: the data generated from a proper per-span `conductors.json` workflow (sections 2–6) would allow the model to distinguish comm from neutral by spatial position and cross-frame trajectory rather than purely by voxel intensity — which is the discriminative signal the current label format does not supply.
