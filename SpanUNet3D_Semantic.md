...
## 8. Incremental retraining on new data

`line_seg/retrain_goal2.py` fine-tunes an existing `best.pt` checkpoint on new span data
without discarding previously learned line-detection parameters.

### 8.1 Why catastrophic forgetting is a real risk

A 3D UNet trained on a finite span corpus memorises spatial patterns at multiple scales.
If new spans are fed with a full learning rate and no constraint, the weights shift rapidly
toward the new distribution and the model "forgets" the old one — IoU on original test spans
drops even as metrics on new spans improve. Three strategies counteract this:

| Strategy | When to use | What it does |
|----------|-------------|-------------|
| **Mixed-data replay** (`--old_data_root`) | Old data is still available | Interleaves new spans with a random subset of old spans every epoch. The model sees both distributions simultaneously. |
| **Frozen encoder** (`--freeze_encoder`) | New data has the same sensor/format | Freezes `enc1` and `enc2` (≈30 % of parameters). Low-level 3D edge/tube detectors are kept intact; only the bottleneck, decoder, and output head adapt. |
| **Elastic Weight Consolidation** (`--ewc_lambda`) | Old data is unavailable | Adds a Fisher-penalised quadratic constraint `λ/2 · Σ F_i (θ_i − θ_i*)²` to the loss. Weights that were critical to old task performance are protected from large updates. |

All three can be used simultaneously for maximum protection.

### 8.2 EWC in detail

The diagonal Fisher Information Matrix is estimated with a single forward–backward
pass over a reference dataset (`--ewc_ref_root`, defaults to `--old_data_root`):

```
F_i  ≈  (1/N) Σ_n  [ ∂ log p(y_n | x_n) / ∂ θ_i ]²
```

Cross-entropy serves as the log-likelihood proxy. The anchor weights `θ*` are the
values loaded from `best.pt`. The penalty is added to every training step before
`loss.backward()`. A typical starting value for `--ewc_lambda` is 5000; reduce to
1000 if new-class metrics plateau, increase to 10000 if old-class IoU drops.

### 8.3 Run commands

**Old data available — replay + frozen encoder (recommended):**

```bash
torchrun --standalone --nproc_per_node=2 -m line_seg.retrain_goal2 \
  --finetune_from ./goal2_runs/exp_v12/best.pt \
  --new_data_root ./new_spans \
  --old_data_root . \
  --old_replay_fraction 0.5 \
  --freeze_encoder \
  --out_dir ./goal2_runs/exp_v12_ft1 \
  --epochs 80 --lr 2e-4 --lr_onecycle \
  --augment --noise_sigma 0.03 \
  --ce_class_weights 1,1,40,10,30,30,30 \
  --dice_class_weights 0.5,1,5,1,3,2,2
```

**Old data NOT available — EWC + frozen encoder:**

```bash
python3 -m line_seg.retrain_goal2 \
  --finetune_from ./goal2_runs/exp_v12/best.pt \
  --new_data_root ./new_spans \
  --ewc_lambda 5000 \
  --freeze_encoder \
...
**Key hyperparameter differences from `train_goal2.py`:**

| Parameter | `train_goal2.py` | `retrain_goal2.py` | Reason |
|-----------|------------------|--------------------|--------|
| `--lr` | 1e-3 | **2e-4** | 5× lower — prevents large weight shifts on first epochs |
| `--epochs` | 157 | **80** | Fewer needed; weights already close to optimum |
| `--onecycle_pct_start` | 0.08 | **0.05** | Shorter warmup — fine-tuning starts close to the optimum |
| `--onecycle_div_factor` | 25 | **10** | Start at lr/10 (not lr/25); avoids over-constraining early steps |
| `--early_stop_patience` | 0 | **25** | Fine-tuning plateaus faster; stop before overfitting new data |

### 8.4 Output files

Identical to `train_goal2.py` outputs, plus one extra file:
...
| `confusion_matrix.png` / `.npy` | Validation confusion matrix |
| `validation_report.txt` | Full evaluation vs GT |
| **`finetune_meta.json`** | Provenance: source checkpoint, strategies used, span counts |

Use `best.pt` directly with `infer_goal2.py` and `export_goal2_onnx.py` — the file format
is identical to a `train_goal2.py` checkpoint.
...
- [`line_seg/retrain_goal2.py`](line_seg/retrain_goal2.py) — incremental fine-tuning script (this section)
