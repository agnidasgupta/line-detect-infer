...
**Implemented**: B&W-only inference — `infer_goal2.py` calls `strip_line_labels_from_input` on every input span before the forward pass, zeroing all label pixels (128–254 → 0) so the model always receives a binary 0/255 volume regardless of whether the input BMPs carry ground-truth labels. No second model or synthetic dataset is required. Output BMPs are written in the standard dataset grayscale encoding (255=air, 0=solid, 128–132=line types), which matches the training label format.

**Not yet included** (future work): instance IDs across frames (stable per-conductor ID across the frame sequence), Goal A binary “has lines” classifier.
...
- **Inference without gray labels**: already implemented. `infer_goal2.py` calls `strip_line_labels_from_input` (same function used in `dataset.py` during training) before every forward pass. Labeled input spans (128–254 in BMPs) and unlabeled spans (purely 0/255) follow identical code paths through the model; the strip is a no-op on already-unlabeled volumes. No second model or synthetic B&W dataset is needed.
